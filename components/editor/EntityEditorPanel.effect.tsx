...
/**
 * EntityEditorPanel - Effect-based version
 *
 * Uses useDocumentService hook instead of React Context.
 * Fully functional with Effect services for character and relationship management.
 */
export function EffectEntityEditorPanel({ open, onClose }: EntityEditorPanelProps) {
  const {
    document,
    addCharacter,
  ...
  const handleRemoveCharacter = async (id: CharacterID) => {
    if (!document) return;
  ...
  const handleAddRelation = async (relation: Omit<Relationship, 'id'>) => {
    if (!document) return;
...
/**
 * EntityEditorPanel with feature flag support
 *
 * Automatically switches between React and Effect implementations
 * based on useEffectEditor feature flag.
 */
export default function EntityEditorPanel(props: EntityEditorPanelProps) {
  const useEffect = typeof window !== 'undefined'
    ? localStorage.getItem('feature-useEffectEditor') === 'true'
    : false;

  if (useEffect) {
    return <EffectEntityEditorPanel {...props} />;
  }

  // Fall back to React version
  // eslint-disable-next-line @typescript-eslint/no-require-imports -- Dynamic require for feature flag
  const ReactEntityEditorPanel = require('./EntityEditorPanel').EntityEditorPanel;
  return <ReactEntityEditorPanel {...props} />;
}
