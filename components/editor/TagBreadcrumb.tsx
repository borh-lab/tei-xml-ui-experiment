...
export function TagBreadcrumb({ onTagSelect }: TagBreadcrumbProps) {
  const [tagHierarchy, setTagHierarchy] = useState<TagInfo[]>([]);
  ...
  // Initialize SelectionManager lazily
  ...
  // Update tag hierarchy when selection changes
  const updateTagHierarchy = useCallback(() => {
    const manager = getSelectionManager();
    const hierarchy = manager.getTagHierarchy();
  ...
  useEffect(() => {
    ...
    return () => {
      document.removeEventListener('click', updateTagHierarchy);
      document.removeEventListener('keyup', updateTagHierarchy);
    };
  }, [updateTagHierarchy]);

  // Handle breadcrumb item click
  const handleTagClick = useCallback((tagInfo: TagInfo, event: React.MouseEvent) => {
