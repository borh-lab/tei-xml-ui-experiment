...
export function EffectStatisticsDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const dialoguePerPassage: ChartData[] = passages.map((passage: any) => ({
      passage: `#${passage.index + 1}`,
      dialogue: passage.tags.filter((t: any) => t.type === 'said').length,
    }));

    // Character with most dialogue
    const characterDialogueCount = new Map<string, number>();
    dialogue.forEach((d: any) => {
      if (d.speaker) {
        const char = characters.find((c: any) => c.id === d.speaker);
...
/**
 * StatisticsDashboard with feature flag support
 *
...
export default function StatisticsDashboard() {
  ...
  if (useEffect) {
    return <EffectStatisticsDashboard />;
  }

  // Fall back to React version (which also uses Effect services internally)
  const ReactStatisticsDashboard = require('./StatisticsDashboard').StatisticsDashboard;
  return <ReactStatisticsDashboard />;
}
