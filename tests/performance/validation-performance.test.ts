...
describe('Validation Performance Tests', () => {
  describe('Cache Effectiveness', () => {
    it('should be significantly faster with cache (warm vs cold)', () => {
      ...
      const cache = new SchemaCache({ maxSize: 10, ttl: 60000 }, (path: string) =>
    ...
    it('should have high cache hit rate with repeated access', () => {
      ...
      console.log(
        `Average cache access time: ${avgTime.toFixed(4)}ms over ${iterations} iterations`
      );
      expect(avgTime).toBeLessThan(1); // Should be < 1ms per access
    });

    it('should cache multiple schemas efficiently', () => {
      const schemas = ['test-simple.rng', 'test-tei.rng'];
  ...
  describe('Validator Throughput', () => {
    ...
    it('should handle >100 validations per second', () => {
      ...
      for (let i = 0; i < validations; i++) {
        validator.validate(passage, testRange, 'persName', { ref: '#char-1' }, document);
      }
      ...
      console.log(
        `Throughput: ${validationsPerSecond.toFixed(0)} validations/second (${validations} validations in ${duration.toFixed(2)}ms)`
      );
    ...
    it('should maintain throughput with different tag types', () => {
      ...
      for (let i = 0; i < validations; i++) {
        ...
        validator.validate(passage, testRange, tagType, { ref: '#char-1' }, document);
      }
      ...
      console.log(
        `Mixed tag type throughput: ${validationsPerSecond.toFixed(0)} validations/second`
      );
      expect(validationsPerSecond).toBeGreaterThan(100);
    });
