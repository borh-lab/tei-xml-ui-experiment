...
test.describe('TEI Dialogue Editor - Real App Tests', () => {
  ...
  test.describe('Sample Gallery', () => {
    ...
    test('should load all available samples from gallery', async ({ page }) => {
      ...
      await expect(page.getByText(/Yellow Wallpaper|Gift of the Magi|Tell-Tale Heart/i).first()).toBeVisible();
  ...
  test.describe('AI Mode Switching', () => {
    ...
    test('should accept and reject AI suggestions', async ({ page }) => {
      ...
      if (hasSuggestions) {
        // Find accept button
        const acceptButton = page.getByRole('button', { name: /Accept suggestion/i }).first();
        ...
        const rejectButton = page.getByRole('button', { name: /Reject suggestion/i }).first();
