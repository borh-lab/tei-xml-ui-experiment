 ...
 */
function extractContext(text: string, quoteIndex: number, quoteLength: number, windowSize: number = 50): string {
...
function calculateConfidence(
  ...
  if (contextLower.includes('chapter') || contextLower.includes('page') ||
      contextLower.includes('written by') || contextLower.includes('author')) {
    confidence -= 0.2;
  }
...
export function nlpDetectDialogue(text: string): DialogueSpan[] {
  ...
  for (const span of spans) {
    const overlaps = filteredSpans.filter(s =>
    ...
    if (overlaps.length === 0) {
      ...
      const minOverlap = overlaps.reduce((min, s) => s.confidence < min ? s.confidence : min, overlaps[0].confidence);
